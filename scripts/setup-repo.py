...
def write_file(dest: Path, content: str, dry_run: bool = False, force: bool = True) -> bool:
    ...
    try:
        ...
        print_step("❌", f"Error writing {dest.name}: {e}")
        return False
...
def setup_repo(
    ...
    # ─────────────────────────────────────────────────────────────────────
    print(f"\n{'=' * 60}")
    print("📋 DRY RUN COMPLETE - No changes made" if dry_run else "✅ SETUP COMPLETE")
    print(f"{'=' * 60}")

    print("\n📂 Files created/updated:")
    print("   pyproject.toml                        → Python tools config")
    ...
    print("\n⚡ Workflow behavior:")
    print("   • Pull Request  → Validation (changed files only, ~5-10s)")
    if gist_id:
        print("   • Weekly (Sun)  → Badge update (full repo scan)")
        print("   • Manual        → Badge update (workflow_dispatch)")
    else:
        print("   • Badges        → Disabled (no gist-id provided)")
...
Examples:
  ...
  python setup-repo.py /path/to/repo --gist-id abc123 --docstring-threshold 70
    ...
    parser.add_argument("--fail-on-warnings", action="store_true", default=True, help="Fail on warnings (default: true)")
    ...
    parser.add_argument("--docstring-threshold", type=int, default=80, help="Docstring coverage threshold (default: 80)")
